 ...
 * This funciton was copied form kdupdaterupdateoperations.cpp
 */
...
void UpdateOperationsTest::testMove()
{
    ...
    // Case #3: Test a valid source -> destination move
    {
        ...
        QVERIFY2(QFile::exists(destFile), "Desination file not found");
...
void UpdateOperationsTest::testExecute()
{
    ...
    {
        const QString program =  QLatin1String( "nonexistingprogram" ) ; // Lets hope this doesnt exist
        FETCH_UPDATE_OPERATION(Execute, QStringList() << program );
        bool success = oper->performOperation();
        printError(oper);
        QVERIFY2(!success, "Execute succeeded in executing a program that doesnt exist");
    }
