...
/*!
 \internal Helper class allowing us to open a QTemporaryFile in other mode than ReadWrite.
 */
...
/*!
  ...
  a softlink. Additionally, it is exception-safe and prevents incompletely written files, i.e. KDSaveFile makes sure that all your
  ...
  KDSaveFile is exception-safe. If there's an exception thrown during writing to KDSaveFile, i.e. between 
  ...
  renames this file to the actual file name when commiting. If the file was already existing, a backup 
  is created, if requested.
*/
...
KDAB_UNITTEST_SIMPLE( KDSaveFile, "kdtools/core" ) {
    const QString filename = QString::fromLatin1( "kdsavefile-test%1" ).arg( QUuid::createUuid() );
