    ...
    */
    ...
    bool useNativeMacPixmaps() const
    {
       ...
       if(!isMacStyle( q->style() ) )
          return false;
       static bool use = QImageReader::supportedImageFormats().contains( QByteArray( "tiff" ) );
       return use;
#else
       return false;
#endif
    ...
    QPixmap pixmap() const {
        ...
        if ( useNativeMacPixmaps() && animation.state() == QTimeLine::Running ) {
            ...
            if ( !findPixmap( anim[animationValue], &pixmap ) ) {
                pixmap.load( QLatin1String( NATIVE_MAC_PATH ) + QString::fromLatin1( "Lock_Open Anim %1.tif" ).arg( animationValue, 2, 10, QLatin1Char( '0' ) ) );
    ...
    QPixmap pixmap( bool locked ) const {
        ...
        if ( !findPixmap( key[locked], &pix ) ) {
            pix.load( useNativeMacPixmaps() ? QString::fromLatin1( NATIVE_MAC_PATH ) + QLatin1String( locked ? "Lock_Locked State.tif"
                                                                                                             : "Lock_Unlocked State.tif" )
...
/*!
 ...
 KDLockButton has a @c text property and a @c lockedText property. The @c text is shown when the button is unlocked. It defaults to "Click the lock to prevent further changes."
 The @c lockedText is shown when the button is locked. It defaults to "Click the lock to make changes."
*/
...
/*!
 ...
 Specifies wheter the button is locked or not. This is the opposite of QAbstractButton's checked and is provided to avoid confustion.
