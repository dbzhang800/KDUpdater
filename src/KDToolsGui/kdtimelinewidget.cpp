...
{
  typedef typename ForwardIter::value_type ValueType;
...
{
    // There was a behavior change of qDeleteAll in Qt 4.8: it doesn't make a copy of the container anymore.
    // That would crash here.
    qDeleteAll( QList<KDTimeLineWidgetItem*>( d->items ) );
}
...
/*! If set to @c true, the current time indicator can only be positioned
  so it is aligned to minor tickmarks. If set to @c false, it can be positioned
  freely in time. */
...
/*! \returns @c true if the current time indicator will snap to
  the minor tickmarks in this timeline and @c false otherwise.
