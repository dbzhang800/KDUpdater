...
/*! Set the input mask of the property. This is used as a hint for the editor.
...
public:
    Private( int value, const QString & name )
        : KDAbstractPropertyBase::Private( value, name ) {}
    ~Private() {}
};
...
KDIntProperty::KDIntProperty(int value, const QString &name)
    : KDAbstractProperty<int>( new Private( value, name ), false )
{
    init( false );
}
...
public:
    Private( double value, const QString & name )
        : KDAbstractPropertyBase::Private( value, name ) {}
    ~Private() {}
};
...
KDDoubleProperty::KDDoubleProperty(double value, const QString &name)
    : KDAbstractProperty<double>( new Private( value, name ), false )
{
    init( false );
}
...
public:
    Private( int min, int max, int val, const QString & name_ )
        : KDIntProperty::Private( val, name_ ), minimum( min ), maximum( max ) {}
    ~Private();
...
QVariant KDIntWithSliderProperty::editorHint(int hint) const
{
  switch(hint) {
  case KDPropertyModel::MinimumHint: return qVariantFromValue(d->minimum);
  case KDPropertyModel::MaximumHint: return qVariantFromValue(d->maximum);
