...
/*!
   ...
   This class is the abstract base class for all tasks in KDUpdater. A task is an activity that
   occupies a certain amount of execution time. It can be started, canceled, paused and
   resumed (Note: a task can be started only once). Tasks can report progress and error messages which an application can show in any
