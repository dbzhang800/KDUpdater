...
function check_license {
    ...
    if [ -f "$SRCDIR/LICENSE.GPL.txt" -a -f "$SRCDIR/LICENSE.US.txt" -a -f "$SRCDIR/LICENSE.txt" ] ; then
        ...
        if [ -f "$SRCDIR/LICENSE.LGPL.txt" ] ; then
    ...
    elif [ -f "$SRCDIR/LICENSE.LGPL.txt" ] ; then
        license="4"
    ...
    if [ "$license" = "1" ]; then
        ...
        license_name="GNU Lesser General Public License (LGPL)"
        license_file=LICENSE.LGPL.txt
    else
        return 1
    fi
...
cat <<EOF 1>&2
  ...
  -compat / -no-compat
      enable/disable source-level compatibility
      with older KD Tools releases

  -kdupdatergui none,textbrowser,webview
      set the GUI mode for KDUpdater, or disable the GUI
...
#	      usage
        ...
        -kdupdatergui)
            shift
            if [ $# -eq 0 ] ; then
                echo "-kdupdatergui needs an argument" 2>&1
                usage
            fi
            kdupdatergui="$1"
            ;;
...
(
    ...
    [ "$compat" = "no" ] && echo "DEFINES += KDTOOLS_NO_COMPAT"
    [ "$kdupdatergui" = "textbrowser" ] && echo "CONFIG += kdupdatergui kdupdaterguitextbrowser"
    [ "$kdupdatergui" = "webview" ] && echo "CONFIG += kdupdatergui kdupdaterguiwebview"
...
if [ ! -f ${product}.pro ] ; then # ie. srcdir != builddir
    if [ -d "$SRCDIR/features" ] ; then
        ...
        for P in "$SRCDIR/features"/*.prf ; do
            name=$(basename "$P")
            echo "include( \"\$\$${PRODUCT}_BASE/features/$name\" )" > "$BUILDDIR/features/$name" || die "Could not create forward .prf file $BUILDDIR/features/$name"
        done
    fi
fi
...
"$QTDIR/bin/qmake" ${SPEC}  "$SRCDIR/$product.pro" "${PRODUCT}_BASE=`pwd`" || die "qmake failed"
...
cat <<EOF 1>&2
  ...
  Older KD Tools Source-Level
    Compatibility............: $compat (default: yes)
  Compile KDUpdater GUI using: $kdupdatergui (default: textbrowser)
